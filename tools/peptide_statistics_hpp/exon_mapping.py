...
#     start_end_pair = [int(l) for l in loc.split("/")]
#     start = min(start_end_pair)
#     end = max(start_end_pair)
#     return (start,end)
#
# def load_exons(gene_lines):
#     exon_map = defaultdict(lambda: defaultdict(list))
#     exon_map_select = defaultdict(lambda: defaultdict(list))
#     for line in gene_lines:
#         sline = line.split("@")
#         chromosome = sline[1]
#         transcript = sline[2]
#         gene = sline[3]
#         for (start,end) in [parse_location(l) for l in sline[-1].split(";")[1:] if l != '']:
#             exon_map[gene][start].append((end,transcript))
#             exon_map_select[gene][transcript].append((start,end))
#     return exon_map, exon_map_select
#
# def load_exon_map(input_peptide_filename):
#     peptides_to_exons = defaultdict(list)
#     # proteins_to_exons = defaultdict(list)
#     with open(input_peptide_filename) as f:
#         r = DictReader(f, delimiter='\t')
#         for line in r:
#             for exon_coverage in [x for x in line['exons'].split(";")]:
#                 split_exon_coverage = exon_coverage.split(":")
#                 chromosome = split_exon_coverage[0]
#                 start_end = split_exon_coverage[1]
#                 start_end_pair = [int(l) for l in start_end.split("/")]
#                 start = min(start_end_pair)
#                 end = max(start_end_pair)
#                 peptides_to_exons[line['peptide'].replace("I", "L")].append((chromosome,(start,end)))
#     return peptides_to_exons
...
def map_peptides_to_exons(exon_fasta, input_sequences):
    ...
    #         location_index = [fasta_seq.find(pep)]
            ...
            for loc in pep_location:
                chrNum = loc[6]#[-1]
                if chrNum not in pepdic[original_pep][1]:
    ...
    for chrNum in coordi_list:
        list_in_chr = list(coordi_list[chrNum].keys())
        list_in_chr.sort()
        grouping_info = []
        gstart = -3000
    ...
    #     Sprob.append(prob)
        ...
        for coord in list_in_chr:
            ...
            for i in pepdic[pep][1].get(chrNum):
                if i[3] != coord:
                    continue
                mapped_exon_portions = []
                complete_exon_loc = []
                for j in range(len(i[0])):
                    mapped_exon_portions.append((i[0][j],i[1][j]))
                    complete_exon_loc.append(i[9][j])
                    # output2.write(str(i[0][j])+'/'+str(i[1][j])+';')
                pep_exons[pep][(chrNum,tuple(mapped_exon_portions),tuple(complete_exon_loc))].append((i[7],i[8]))
